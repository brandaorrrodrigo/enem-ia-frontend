...
const PRECO_MENSAL = 39.90;
const PRECO_ANUAL = 399.00;
...
function AssinaturaProContent() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-emerald-900 to-emerald-950 px-4 py-8">
      <div className="mx-auto max-w-2xl">
        <Link href="/planos" className="text-emerald-300 hover:text-white text-sm mb-4 inline-block">
          ← Voltar aos planos
        </Link>

        <h1 className="text-3xl md:text-4xl font-extrabold text-white">Assinar ENEM PRO</h1>
        <p className="mt-2 text-emerald-200">
          O plano mais escolhido pelos estudantes
        </p>
        ...
        <div className="mt-6 rounded-2xl bg-emerald-800/50 ring-1 ring-emerald-600 p-5 shadow-lg">
          <div className="text-center mb-6">
            <div className="text-4xl font-bold text-yellow-400">
              R$ {precoAtual.toFixed(2)}
            </div>
            <div className="text-emerald-300 text-sm">{periodoTexto}</div>
          </div>

          <div className="grid gap-3">
            <input
              className="rounded-lg border border-emerald-600 bg-emerald-900 px-3 py-2 text-sm text-white placeholder-emerald-400"
              ...
              <div className="mt-2 flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className={`rounded-xl px-4 py-3 font-semibold ring-1 transition ${
                    metodo === 'pix'
                      ? 'bg-yellow-500 text-emerald-900 ring-yellow-500'
                      : 'bg-emerald-900 text-white ring-emerald-600 hover:bg-emerald-800'
                  }`}
                >
                  PIX (recomendado)
                </button>
            ...
            {/* PIX */}
            {metodo === 'pix' && (
              <div className="mt-4 rounded-xl bg-emerald-900 p-4 ring-1 ring-emerald-600">
                <p className="text-sm text-emerald-200">
                  Envie <b className="text-yellow-400">R$ {precoAtual.toFixed(2)}</b> para a chave PIX:
                </p>
                <p className="mt-2 select-all rounded-lg bg-emerald-950 p-3 font-mono text-sm ring-1 ring-emerald-600 text-white">
                  {PIX_KEY}
                </p>
          ...
          <ul className="mt-6 text-sm text-emerald-200 list-disc pl-5 space-y-1">
...
export default function AssinaturaProPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-emerald-900 flex items-center justify-center"><div className="text-white">Carregando...</div></div>}>
